...
function partiyCode() {
    ...
    switch (select.value) {
        ...
        case '16':
            ...
            var p2 = data[0] ^ data[2] ^ data[3] ^ data[5] ^ data[6] ^ data[9] ^ data[10] ^ data[12] ^ data[13];
            var p4 = data[1] ^ data[2] ^ data[3] ^ data[7] ^ data[8] ^ data[9] ^ data[10] ^ data[14] ^ data[15];
            ...
            <tr><td class='text-danger'><u><b>?</b></u></td><td class='text-danger'><u><b>?</b></u></td><td>${data[0]}</td><td class='text-danger'><u><b>?</b></u></td><td>${data[1]}</td><td>${data[2]}</td><td>${data[3]}</td><td class='text-danger'><u><b>?</b></u></td><td>${data[4]}</td><td>${data[5]}</td><td>${data[6]}</td><td>${data[7]}</td><td>${data[8]}</td><td>${data[9]}</td><td>${data[10]}</td><td class='text-danger'><u><b>?</b></u></td><td>${data[11]}</td><td>${data[12]}</td><td>${data[13]}</td><td>${data[14]}</td><td>${data[15]}</td></tr>
            
         </table>
         <p style='font-size:25px;'>
         p1= p3 ⊕ p5 ⊕ p7 ⊕ p9 ⊕ p11 ⊕ p13 ⊕ p15 ⊕ p17 ⊕ p19 ⊕ p21 = ${p1} <br>
        p2= p3 ⊕ p6 ⊕ p7 ⊕ p10 ⊕ p11 ⊕ p14 ⊕ p15 ⊕ p18 ⊕ p19 = ${p2} <br>
        p4= p5 ⊕ p6 ⊕ p7 ⊕ p12 ⊕ p13 ⊕ p14 ⊕ p15 ⊕ p20 ⊕ p21 = ${p4} <br>
         p8= p9 ⊕ p10 ⊕ p11 ⊕ p12 ⊕ p13 ⊕ p14 ⊕ p15= ${p8} <br>
         p16= p17 ⊕ p18 ⊕ p19 ⊕ p20 ⊕ p21= ${p16} <br></p>
            <h6 class="display-6"> Parity bitleri p1: ${p1},  p2: ${p2},  p4: ${p4}</h6>
            <table class='table table-bordered text-white'>
               <tr><td><u><b>p1</b></u></td><td><u><b>p2</b></u></td><td>p3</td><td><u><b>p4</b></u></td><td>p5</td><td>p6</td><td>p7</td><td><u><b>p8</b></u></td><td>p9</td><td>p10</td><td>p11</td><td>p12</td><td>p13</td><td>p14</td><td>p15</td><td><u><b>p16</b></u></td><td>p17</td><td>p18</td><td>p19</td><td>p20</td><td>p21</td></tr>
               <tr><td class='text-danger'><u><b>${p1}</b></u></td><td class='text-danger'><u><b>${p2}</b></u></td><td>${data[0]}</td><td class='text-danger'><u><b>${p4}</b></u></td><td>${data[1]}</td><td>${data[2]}</td><td>${data[3]}</td><td class='text-danger'><u><b>${p8}</b></u></td><td>${data[4]}</td><td>${data[5]}</td><td>${data[6]}</td><td>${data[7]}</td><td>${data[8]}</td><td>${data[9]}</td><td>${data[10]}</td><td class='text-danger'><u><b>${p16}</b></u></td><td>${data[11]}</td><td>${data[12]}</td><td>${data[13]}</td><td>${data[14]}</td><td>${data[15]}</td></tr>
    ...
    switch (bit) {
        ...
        case 16:
            ...
            if (c != 0) {
                ...
                <tr><td>${pCheck[0]}</td><td>${pCheck[1]}</td><td>${pCheck[2]}</td><td>${pCheck[3]}</td><td>${pCheck[4]}</td><td>${pCheck[5]}</td><td>${pCheck[6]}</td><td>${pCheck[7]}</td><td>${pCheck[8]}</td><td>${pCheck[9]}</td><td>${pCheck[10]}</td><td>${pCheck[11]}</td><td>${pCheck[12]}</td><td>${pCheck[13]}</td><td>${pCheck[14]}</td><td>${pCheck[15]}</td><td>${pCheck[16]}</td><td>${pCheck[17]}</td><td>${pCheck[18]}</td><td>${pCheck[19]}</td><td>${pCheck[20]}</td></tr></table>
                <p style='font-size:25px;'>
                c1 = p1 ⊕ p3 ⊕ p5 ⊕ p7 ⊕ p9 ⊕ p11 ⊕ p13 ⊕ p15 ⊕ p17 ⊕ p19 ⊕ p21 = ${c1} <br>
                c2 = p2 ⊕ p3 ⊕ p6 ⊕ p7 ⊕ p10 ⊕ p11 ⊕ p14 ⊕ p15 ⊕ p18 ⊕ p19 = ${c2}<br>
                c4 = p4 ⊕ p5 ⊕ p6 ⊕ p7 ⊕ p12 ⊕ p13 ⊕ p14 ⊕ p15 ⊕ p20 ⊕ p21 = ${c4}<br>
                c8 = p8 ⊕ p9 ⊕ p10 ⊕ p11 ⊕ p12 ⊕ p13 ⊕ p14 ⊕ p15 = ${c8}<br>
                c16= p16 ⊕ p17 ⊕ p18 ⊕ p19 ⊕ p20 ⊕ p21 = ${c16}<br></p>
                <h6 class='display-6'>Check bitleri c16: ${c16}  c8: ${c8} c4: ${c4},  c2: ${c2},  c1: ${c1} - C: ${c16}${c8}${c4}${c2}${c1}  <br> Check bitlerinden hatanın p${c} bitinde olduğunu doğrulayabiliriz.</h6>
                `
            }
            break;
        default:
